...
namespace dgr582 {
    ...
    private:
        ...
        unsigned int polygoniseCell(size_t i, size_t j, size_t k, double isoLevel,
                                    std::array<dgr582::Triangle3D, 5> &triangleResult);

        std::array<size_t, 3> cellCornerIndexToIJKIndex(size_t vertexIndex, size_t i, size_t j, size_t k);

        dgr582::Point3D interpolateVertex(double isoLevel, const std::array<size_t, 3> &vertIJK1,
                                          const std::array<size_t, 3> &vertIJK2);
