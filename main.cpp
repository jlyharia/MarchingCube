...
#include "headers/latticeData3D.h"
#include "headers/cubeLatticeScalarField3D.h"
#include "headers/sphere.h"
#include "headers/shaders.h"
#include "headers/shadersDefinitions.h"
    ...
    for (size_t i = 0; i < nX; ++i) {
        for (size_t j = 0; j < nY; ++j) {
            for (size_t k = 0; k < nZ; ++k) {
                ...
                auto perpDist = computeDistanceAFromBC(glm::vec3(dX, dY, dZ),
...
//    glfwSetScrollCallback(window, scroll_callback);
    ...
    //init shaders
    marchingCubesShaderProgram = dgr582::prepareShaders(dgr582::marchingCubesVertexShaderSource,
                                                        dgr582::marchingCubesFragmentShaderSource);
    lightSphereShaderProgram = dgr582::prepareShaders(dgr582::lightSourceVertexShaderSource,
                                                      dgr582::lightSourceFragmentShaderSource);


    //Prepare data
    dgr582::Sphere lightSphere{1.0f, 36, 18};
    lightSphere.printSelf();
    ...
    glUseProgram(marchingCubesShaderProgram);
    glUniformMatrix4fv(glGetUniformLocation(marchingCubesShaderProgram, "projection"), 1, GL_FALSE,
                       glm::value_ptr(projection));

    glUseProgram(lightSphereShaderProgram);
    glUniformMatrix4fv(glGetUniformLocation(lightSphereShaderProgram, "projection"), 1, GL_FALSE,
                       glm::value_ptr(projection));
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUniformMatrix4fv(glGetUniformLocation(marchingCubesShaderProgram, "model"), 1, GL_FALSE,
                           glm::value_ptr(marchingCubesModelMat));
        ...
        glUniformMatrix4fv(glGetUniformLocation(lightSphereShaderProgram, "model"), 1, GL_FALSE,
                           glm::value_ptr(lightSourceModelMat));
